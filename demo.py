...
def get_args():
    ...
    parser.add_argument('--ratio', type=int, default=1/2, help='the masking ratio parameter')
    ...
    parser.add_argument('--exp', type=int, default=10, help='Which experiment are you running')
...
class Dataset():
    def __init__(self, path, args):
        ...
        Dataset class comprises of:
            data: [num_subjects, num_timepoints, num_regions, num_regions]
            args
            path
        """

        self.path = path
        file_t0 = self.path + '0.mat'
        file_t1 = self.path + '1.mat'
        file_t2 = self.path + '2.mat'
        subs_0 = io.loadmat(file_t0)  # sub_i is a dict
        subs_1 = io.loadmat(file_t1)
        subs_2 = io.loadmat(file_t2)
        subs_0 = torch.from_numpy(np.delete(np.array(subs_0['data_t0']), 88, 0))
        subs_1 = torch.from_numpy(np.delete(np.array(subs_1['data_t1']), 88, 0))
        subs_2 = torch.from_numpy(np.delete(np.array(subs_2['data_t2']), 88, 0))
        self.args = args

        x = torch.zeros((len(subs_0), 1, self.args.num_regions, self.args.num_regions))
        y = torch.zeros((len(subs_0), 1, self.args.num_regions, self.args.num_regions))
        y2 = torch.zeros((len(subs_0), 1, self.args.num_regions, self.args.num_regions))
        for i in range(len(subs_0)):
            index = 0
            for j in range(0, self.args.num_regions):  # every iteration j decrements by 1, 34, 33, 32, ..., 1 = 595
                x[i, 0, j, j + 1:self.args.num_regions] = subs_0[i,
                                                    index:index + self.args.num_regions - j - 1]  # place the features where they belong
                x[i, 0, j, j] = 0  # place zero in diagonal
                x[i, 0, j + 1:self.args.num_regions, j] = subs_0[i, index:index + self.args.num_regions - j - 1]
                y[i, 0, j, j + 1:self.args.num_regions] = subs_1[i,
                                                    index:index + self.args.num_regions - j - 1]  # place the features where they belong
                y[i, 0, j, j] = 0  # place zero in diagonal
                y[i, 0, j + 1:self.args.num_regions, j] = subs_1[i, index:index + self.args.num_regions - j - 1]
                y2[i, 0, j, j + 1:self.args.num_regions] = subs_2[i,
                                                     index:index + self.args.num_regions - j - 1]  # place the features where they belong
                y2[i, 0, j, j] = 0  # place zero in diagonal
                y2[i, 0, j + 1:self.args.num_regions, j] = subs_2[i, index:index + self.args.num_regions - j - 1]
                index = index + self.args.num_regions - j - 1

        self.data = torch.cat((x, y, y2), 1)
...
if __name__ == "__main__":
    ...
    table = random_table(args, args.ratio)
    print(table)
